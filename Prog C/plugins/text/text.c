...
char *encode_text(char *key, char *text) {
    ...
    for(int i = 0; i < 1+length/16; i++) {
        ...
        if((length*2 - i*32) <= 16) {
            ...
            encode_blockstr(notfullbloc, key_expended, ciphered);
        }
        else {
            encode_blockstr(&texthex[i*32], key_expended, ciphered);   
        }    
...
char *decode_text(char *key, char *text) {
    ...
    for(int i = 0; i < length/32; i++) {
        ...
        if((length - i*32) <= 32) {
            ...
            decode_blockstr(notfullbloc, key_expended, unciphered);
        }
        else {
            decode_blockstr(&text[i*32], key_expended, unciphered);
        }
